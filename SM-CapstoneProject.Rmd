...
This project is related to the  HervardX: PH125.9x Data Science: Capstone course MovieLens Project. The present report start with a general idea of the project and by representing its objectives.

Then the given movie lens dataset will be loaded and setup. An exploratory data analysis is carried out in order to develop a machine learning algorithm that could predict movie ratings until a final model. Results will be explained. Finally the report ends with some concluding remarks.
...
In order to predict in the most possible accurate way the movie rating of the users that haven’t seen the movie yet, the he MovieLens dataset will be splitted into 2 subsets that will be the “edx”, a training subset to train the algorithm, and “validation” a subset to test the movie ratings.  "edx' and "validation" dataset were downloaded from edX site and loaded to Studio before running this script.
...
To get familiar with the dataset given by edX, we find the first rows of "edx" subset as below.
...
We can conclude that a machine learning algorithm is built to predict movie ratings with edX Capstone MovieLens dataset.
The regularized model including the effect of user is characterized by the lower RMSE value and is hence the optimal model to use for the capstone movielens project.
The optimal model characterised by the lowest RMSE value (0.8648170) lower than the initial evaluation criteria (0.8775) given by the goal of the capstone movielens project.
We could also confirm that improvements in the RMSE could be achieved by adding other effect (genre, year, age,..). Other different machine learning models could also improve the results further, but hardware limitations and the main memory, are the constraints to further optimization.
